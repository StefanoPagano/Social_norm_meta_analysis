# set wd 
setwd("/Volumes/GoogleDrive/.shortcut-targets-by-id/1IoJDOQWCFiL1qTzSja6byrAlCelNSTsT/Meta-analysis beliefs/Dati paper/2015Ves034")
...
norms <- ug %>% 
  ...
  mutate(AR0_M = dplyr::recode(AR0, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR1_M = dplyr::recode(AR1, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR2_M = dplyr::recode(AR2, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR3_M = dplyr::recode(AR3, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR4_M = dplyr::recode(AR4, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR5_M = dplyr::recode(AR5, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR6_M = dplyr::recode(AR6, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR7_M = dplyr::recode(AR7, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR8_M = dplyr::recode(AR8, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR9_M = dplyr::recode(AR9, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1),
         AR10_M = dplyr::recode(AR10, `1`= -1, `2` = -1/3, `3` = 1/3, `4` = 1))

# meta-information dataset
meta_dataset <- read_xlsx(path = "/Volumes/GoogleDrive/.shortcut-targets-by-id/1IoJDOQWCFiL1qTzSja6byrAlCelNSTsT/Meta-analysis beliefs/Social Norms meta.xlsx", sheet = "ALL") %>% subset.data.frame(subset = PaperID == "2015Ves034", select = c(n_Paper, PaperID, TreatmentCode, TreatmentName_paper, Year, Outlet, Published, FirstTask, between_vs_within, Game_type, Standard_game, Baseline, Group_size, One_Shot_Repeated, Choice_Method, Matching, Rounds, Punishment, Rewards, Monetary_Incentivized_experiment, Environment, Method_elicitation, Separate_sample_beliefs, Belief_repeated, Before_after_main_decisions, KW_Normative, KW_Personal, Bicchieri_Empirical, Bicchieri_Normative, Bicchieri_Personal_Beliefs, Bicchieri_between, Incentives_beliefs, StatusTreatment_Roma)) %>% mutate(TreatmentCode = as.numeric(TreatmentCode))
...
db_appropriateness <- ug1_appropriateness_sum %>% select(donation, Kw_m) %>% mutate(PaperID = "2015Ves034", TreatmentCode = 1)
...
db_appropriateness <- ug2_appropriateness_sum %>% select(donation, Kw_m) %>% mutate(PaperID = "2015Ves034", TreatmentCode = 4) %>%
