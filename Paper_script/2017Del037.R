...
meta_dataset <- read_xlsx(path = "/Volumes/GoogleDrive/.shortcut-targets-by-id/1IoJDOQWCFiL1qTzSja6byrAlCelNSTsT/Meta-analysis beliefs/Social Norms meta.xlsx", sheet = "ALL") %>% subset.data.frame(subset = PaperID == "2017Del037", select = c(n_Paper, PaperID, TreatmentCode, TreatmentName_paper, Year, Outlet, Published, FirstTask, between_vs_within, Game_type, Standard_game, Baseline, Group_size, One_Shot_Repeated, Choice_Method, Matching, Rounds, Punishment, Rewards, Monetary_Incentivized_experiment, Environment, Method_elicitation, Separate_sample_beliefs, Belief_repeated, Before_after_main_decisions, KW_Normative, KW_Personal, Bicchieri_Empirical, Bicchieri_Normative, Bicchieri_Personal_Beliefs, Bicchieri_between, Incentives_beliefs, StatusTreatment_Roma)) %>% mutate(TreatmentCode = as.numeric(TreatmentCode))
...
if (min(dg_base_appropriateness_sum$Kw_m) < 0){
  
  negative_appropriateness <- dg_base_appropriateness_sum %>% subset.data.frame(subset = Kw_m < 0) %>% 
    mutate(abs_Kw_m = abs(Kw_m), delta_max = max(Kw_m) - Kw_m)
  
} else {
  
  negative_appropriateness <- dg_base_appropriateness_sum %>% mutate(delta_max = 0)
...
dg_base_final_norms <- merge.data.frame(dg_base_appropriateness_sum, dg_base_norms_var, by = "donation") %>% 
  ...
  mutate(PaperID = "2017Del037", 
         ...
         max_sigma = val_max_sigma) %>%
  subset.data.frame(select = -c(..x, ..y, donation))

# 3. combine dataset ----
finaldf <- meta_dataset %>% 
  merge.data.frame(dg_base_dta_coop, by = c("PaperID","TreatmentCode")) %>%
  merge.data.frame(dg_base_final_norms[2,], all.x=T, by = c("PaperID","TreatmentCode")) %>% 
...
if (min(dg_in_appropriateness_sum$Kw_m) < 0){
  
  negative_appropriateness <- dg_in_appropriateness_sum %>% subset.data.frame(subset = Kw_m < 0) %>% 
    mutate(abs_Kw_m = abs(Kw_m), delta_max = max(Kw_m) - Kw_m)
  
} else {
  
  negative_appropriateness <- dg_in_appropriateness_sum %>% mutate(delta_max = 0)
...
if (min(dg_out_appropriateness_sum$Kw_m) < 0){
  
  negative_appropriateness <- dg_out_appropriateness_sum %>% subset.data.frame(subset = Kw_m < 0) %>% 
    mutate(abs_Kw_m = abs(Kw_m), delta_max = max(Kw_m) - Kw_m)
  
} else {
  
  negative_appropriateness <- dg_out_appropriateness_sum %>% mutate(delta_max = 0)
