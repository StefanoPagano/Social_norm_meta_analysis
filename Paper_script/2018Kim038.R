# set wd 
setwd("/Volumes/GoogleDrive/.shortcut-targets-by-id/1IoJDOQWCFiL1qTzSja6byrAlCelNSTsT/Meta-analysis beliefs/Dati paper/2018Kim038")
...
meta_dataset <- read_xlsx(path = "/Volumes/GoogleDrive/.shortcut-targets-by-id/1IoJDOQWCFiL1qTzSja6byrAlCelNSTsT/Meta-analysis beliefs/Social Norms meta.xlsx", sheet = "ALL") %>% subset.data.frame(subset = PaperID == "2018Kim038", select = c(n_Paper, PaperID, TreatmentCode, TreatmentName_paper, Year, Outlet, Published, FirstTask, between_vs_within, Game_type, Standard_game, Baseline, Group_size, One_Shot_Repeated, Choice_Method, Matching, Rounds, Punishment, Rewards, Monetary_Incentivized_experiment, Environment, Method_elicitation, Separate_sample_beliefs, Belief_repeated, Before_after_main_decisions, KW_Normative, KW_Personal, Bicchieri_Empirical, Bicchieri_Normative, Bicchieri_Personal_Beliefs, Bicchieri_between, Incentives_beliefs, StatusTreatment_Roma)) #%>% mutate(TreatmentCode = as.numeric(TreatmentCode))
...
# 2. Beliefs dataframe ----
dg_neutral_final_norms <- data.frame(Avg_NE = NA, Var_NE = NA, Sd_Avg_NE = NA, Sd_Avg_NE_min_max = NA, specificity_plus = NA, specificity_min = NA, max_sigma = NA, Avg_KW_m = NA) %>%
  mutate(PaperID = "2018Kim038", TreatmentCode = 1)
...
# 2. Beliefs dataframe ----
dg_tog_final_norms <- data.frame(Avg_NE = NA, Var_NE = NA, Sd_Avg_NE = NA, Sd_Avg_NE_min_max = NA, specificity_plus = NA, specificity_min = NA, max_sigma = NA, Avg_KW_m = NA) %>%
  mutate(PaperID = "2018Kim038", TreatmentCode = 2)
