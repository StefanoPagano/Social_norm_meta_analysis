...
# choice file
dg=read_excel("____", sheet = "___")
# only columns needed
dg <- dg %>% subset.data.frame(select = c(1,4,19:20))

# rename columns
colnames(dg) <- c("___", "___", "___", "___")

# add id variable as a progressive numbers
dg <- dg %>% mutate(id = c(1:374))

# norms file
norms=read_excel("___", sheet = "___")

# rename columns
colnames(norms) <- c("___", "___", "___", "___", "___", "___", "___", "___", )
...
dg_dta_coop <- dg %>% subset.data.frame(select = coldg, Treatment == 1) %>%
  ...
  mutate(PaperID = "2020Bas115", TreatmentCode = 1)
