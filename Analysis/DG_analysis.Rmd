...
for (i in list_treatment) {
  ...
  # list
  p <- ggplot(rbind.data.frame(temp_choice %>% select(treatment_id,choice, percentage, model), temp_predict_se, temp_predict_ia, temp_predict_sqr), 
             aes(x=choice, 
...
summary(lmer(delta ~ ne_avg + (1|treatment), pgg_t))
summary(lm(delta ~ ne_avg + ee_avg + pnb, data = pgg_t))
