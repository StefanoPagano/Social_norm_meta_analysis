...
comparison_aic = function(treatment) {
  ...
  inequality_aversion_sqr_model=clogit(A ~ other_payoff_ahead + other_payoff_behind + sqr_other_payoff_ahead + sqr_other_payoff_behind + strata(subject_id), 
                                  data=df_master)
  ...
  inequality_sqr_delta = as.numeric(inequality_aversion_sqr_model$coefficients[3])
  inequality_sqr_gamma = as.numeric(inequality_aversion_sqr_model$coefficients[4])
  ...
  inequality_sqr_delta_se = coef(summary(inequality_aversion_sqr_model))[3, "se(coef)"]
  inequality_sqr_gamma_se = coef(summary(inequality_aversion_sqr_model))[4, "se(coef)"]
  ...
  paper_and_aic_coeff <- list(treatment, 
                              ...
                              inequality_sqr_beta,
                              inequality_sqr_delta,
                              inequality_sqr_gamma,
                              inequality_sqr_alpha_se,
                              inequality_sqr_beta_se,
                              inequality_sqr_delta_se,
                              inequality_sqr_gamma_se,
                              models)
...
for (t in levels(as.factor(utility_df_treatment_aic_analysis$treatment_id))) {
  ...
  social_coeff <- social_coeff %>% 
    rbind.data.frame(data.frame(treatment_id = treatment, 
                                ...
                                ia_sqr_delta = x$inequality_sqr_delta,
                                ia_sqr_gamma= x$inequality_sqr_gamma,
                                ia_sqr_SE_alpha= x$inequality_sqr_alpha_se,
                                ia_sqr_SE_beta= x$inequality_sqr_beta_se,
                                ia_sqr_SE_delta= x$inequality_sqr_delta_se,
                                ia_sqr_SE_gamma= x$inequality_sqr_gamma_se,
...
social_coeff <- social_coeff %>%
  ...
  mutate(i_SE_beta=ifelse(i_SE_beta>100, NA, i_SE_beta),
         ...
         ia_sqr_SE_gamma=ifelse(ia_sqr_SE_gamma>100, NA, ia_sqr_SE_gamma)) 

average_coeff <- social_coeff %>% summarise(across(i_alpha:s_SE_gamma, ~weighted.mean(., w = n, na.rm=T)))

social_coeff <- social_coeff %>%
   add_row(treatment_id="Average", average_coeff , Game_type="Average", n=sum(social_coeff$n))
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment="", fig.height=7}
...
plot_ia_sqr_delta <- ggplot(social_coeff, aes(x=ia_sqr_delta,y=fct_rev(fct_reorder(treatment_id, Game_type)))) + 
  geom_pointrange(aes(xmin=ia_sqr_delta-2*ia_sqr_SE_delta,xmax=ia_sqr_delta+2*ia_sqr_SE_delta, color=Game_type), shape=20) +
...
plot_ia_sqr_gamma <- ggplot(social_coeff, aes(x=ia_sqr_gamma,y=fct_rev(fct_reorder(treatment_id, Game_type)))) + 
  geom_pointrange(aes(xmin=ia_sqr_gamma-2*ia_sqr_SE_gamma,xmax=ia_sqr_gamma+2*ia_sqr_SE_gamma, color=Game_type), shape=20) +
...
ggarrange(plot_ia_sqr_alpha, plot_ia_sqr_delta, plot_ia_sqr_beta, plot_ia_sqr_gamma, ncol = 2, nrow = 2, common.legend=T, legend="bottom", widths = c(8,6))
```