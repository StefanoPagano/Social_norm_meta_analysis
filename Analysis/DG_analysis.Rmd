...
# beliefs_temp <- beliefs %>%
#   subset.data.frame(subset = Design=="Between"& Game_type=="DG") %>%
#   group_by(paper_id, treatment_id,scenarios) %>%
#   summarise(avg_kw = mean(KW_Normative, rm.na=T),
#             sd_kw = sd(KW_Normative, na.rm = T))
# 
# Kim003_interpolation <- beliefs_temp %>%
...
#   merge.data.frame(Kim003_interpolation, all = T) %>%
#   group_by(paper_id, treatment_id, scenarios) %>%
#   summarise(mean_app=mean(KW_Normative),
#             sd_app=mean(sd_KW))
# 
# individual_db <- choices %>%
...
betas_standard_logit <- data.frame(paper = NA, alpha=NA, beta=NA, SE_alpha=NA, SE_beta=NA)
...
for (i in levels(as.factor(utility_df_treatment$treatment_id))) {
  ...
  print(i)
  print(summary(model))
  betas_standard_logit <- betas_standard_logit %>% rbind.data.frame(data.frame(paper = i, 
...
betas_spec_logit <- data.frame(paper = NA, alpha=NA, beta=NA, delta=NA, SE_alpha=NA, SE_beta=NA, SE_delta=NA)
...
for (i in levels(as.factor(utility_df_treatment$treatment_id))) {
  ...
  betas_spec_logit <- betas_spec_logit %>% rbind.data.frame(data.frame(paper = i, 
...
ggplot(betas_spec_logit %>% filter(!(is.na(beta))) %>% arrange(beta), aes(x=beta,y=paper)) + 
  geom_point() + 
  geom_errorbar(aes(xmin=beta-2*SE_beta,xmax=beta+2*SE_beta))
```
