...
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment="", fig.height=5}
...
pgg_t <- read.csv("/Volumes/GoogleDrive/.shortcut-targets-by-id/1IoJDOQWCFiL1qTzSja6byrAlCelNSTsT/Meta-analysis beliefs/Dati paper/2021Sze167/full.csv") %>% 
  ...
  rowwise() %>%
  mutate(ee_sd = sd(c(ee1:ee5), na.rm=T)/100,
         ne_sd = sd(c(ne1:ne5), na.rm=T)/100,
         delta = coop-(ne_avg/100)) 
  

ml.ne <- lmer(delta ~ 0 + rescale(ne_avg/100) + rescale(ne_sd) + (1|treatment), pgg_t)
ml.ee <- lmer(delta ~ 0 + rescale(ee_avg/100) + rescale(ee_sd) + (1|treatment), pgg_t)
ml.pnb <- lmer(delta ~ 0 + rescale(pnb/100) + (1|treatment), pgg_t)
ml.all <- lmer(delta ~ 0 + rescale(ne_avg/100) + rescale(ee_avg/100) + rescale(pnb/100) + rescale(ee_sd) + rescale(ne_sd) + (1|treatment), pgg_t)
...
tab_model(ml.ne, ml.ee, ml.pnb, ml.all, dv.labels = c("NE", "EE", "PNB", "ALL"), show.aic = T, show.loglik = T, show.se = T, show.ci = F, title = "Model Estimates", transform=NULL)
