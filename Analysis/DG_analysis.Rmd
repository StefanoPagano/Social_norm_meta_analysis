...
df_avg_kw <- read.csv("2019Cha026_avg_kw.csv", sep = ",") %>% mutate(donation=10-donation) %>% rbind.data.frame(df_avg_kw)
...
utility_df_treatment_cha <- utility_df_treatment %>%
  filter(treatment_id == "2019Cha026_1") %>%
  mutate(choice = 10-choice)

utility_df_treatment <- utility_df_treatment %>%
  filter(paper_id != "2019Cha026") %>%
  rbind.data.frame(utility_df_treatment_cha)
...
ggplot(betas_cons_logit %>% filter(!(is.na(delta))) %>% arrange(delta), aes(x=delta,y=paper)) + 
  geom_point() + 
  geom_errorbar(aes(xmin=delta-2*SE_beta,xmax=delta+2*SE_beta))
```
