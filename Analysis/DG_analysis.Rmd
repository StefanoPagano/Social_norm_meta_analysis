...
for (i in list_treatment) {
  ...
  temp_choice <- df_real %>% 
    filter(treatment_id == i)
  temp_model <- models[[i]]
  data.frame(scenarios=,pre=predict(temp_model[[1]], type = "expected")[1:11]*100, scen=c(0:10))
  # list
  list_plot[[n]] <- ggplot() + 
                    geom_bar(temp_choice, 
                             aes(x=choice, 
                                 y=percentage),
                             stat = "identity",
                             alpha=0.5) + 
                    geom_bar(temp_models[[1]],
                             aes(x=scenarios_treatment %>% filter(treatment_id==t) %>% select(n_scen),
                                 y=predict(temp_models[[1]][1:max(scenarios_treatment %>% filter(treatment_id==t) %>% select(n_scen))])),
                             stat = "identity") +
                    labs(title = i, 
                         subtitle = temp_master_for$TreatmentName_paper[temp_master_for$treatment_id == i]) +
