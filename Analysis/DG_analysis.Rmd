...
df_avg_kw <-  df_avg_kw %>% merge.data.frame(master %>% subset.data.frame(select = c(PaperID, TreatmentName_paper, TreatmentCode, treatment_id)), by = c("PaperID","TreatmentCode","treatment_id")) %>% filter(!(is.na(TreatmentName_paper)))
...
perc_choices <-  perc_choices %>% merge.data.frame(master %>% subset.data.frame(select = c(PaperID, TreatmentName_paper, TreatmentCode, treatment_id)), by = "treatment_id") %>% filter(!(is.na(TreatmentName_paper)))
...
df_consistency_beliefs <- beliefs %>% 
  ...
  merge.data.frame(master %>% subset.data.frame(select = c(treatment_id, max_sigma)), by="treatment_id") %>%
  group_by(treatment_id, scenarios) %>% summarise(consistency = sd(KW_Normative), appropriateness = sum(KW_Normative))

tab_max <- beliefs %>% group_by(treatment_id, scenarios) %>% summarise(KW=mean(KW_Normative)) %>% summarise(scenarios=scenarios[which(KW==max(KW))])

output <- merge.data.frame(tab_max, beliefs, all.x = T, by=c("treatment_id", "scenarios"))

Kim003_interpolation <- beliefs %>% subset.data.frame(paper_id == "2016Kim003" & treatment_id == "2016Kim003_7") %>%
...
ggplot(output, aes(x=treatment_id, y=KW_Normative)) + 
  geom_boxplot() + 
  labs(title="Boxplot of consistency per paper") +
  xlab("Paper") + ylab("Consistency")

ggplot(output, aes(x=KW_Normative)) + 
  geom_histogram() + facet_wrap(~treatment_id, ncol=2)

ggplot(output, aes(x=KW_Normative)) + 
  geom_density() + facet_wrap(~treatment_id, ncol=2)
...
df_specificity_per_subject <- beliefs %>% 
  group_by(subject_id) %>% 
  mutate(n_actions_positive = sum(KW_Normative>0), n_actions_negative=sum(KW_Normative<=0), specificity = n_actions_negative/n_actions_positive)

ggplot(df_specificity_per_subject, aes(x=treatment_id, y=specificity)) + 
  geom_boxplot() + 
  labs(title="Boxplot of specificity per paper") +
  xlab("Paper") + ylab("Specificity")

ggplot(df_specificity_per_subject, aes(x=n_actions_positive, y=specificity)) + geom_point()
