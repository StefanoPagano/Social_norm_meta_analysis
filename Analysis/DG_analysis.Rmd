...
for (i in temp_master$treatment_id) {
  ...
  list_plot[[n]] <- ggplot(temp_master_for, 
    ...
    labs(title = i, 
         subtitle = temp_master_for$TreatmentName_paper[temp_master_for$treatment_id == i]) +
    xlim(0,temp_master_for$endowment[1]) +
    scale_x_continuous(breaks = 0:temp_master_for$endowment[1]) +
    ylim(0,100) +
    xlab("Choice") + 
    ylab("Subjects (%)")
...
ggplot(output,
       aes(x=treatment_id,
           y=KW_Normative)) +
  geom_boxplot() +
  labs(title="Boxplot of appropriatensess per paper",
       subtitle = "Consistency can be evaluate throught IQR") +
  xlab("Paper") +
