...
betas_se_logit <- data.frame(paper = NA, delta=NA, gamma=NA, SE_alpha=NA, SE_beta=NA, MRS=NA)
...
comparison_aic = function(treatment) {
  ...
  paper_and_aic <- list(treatment, aic_comp_t)
  names(paper_and_aic) <- c("paper","aic")
  invisible(paper_and_aic)
...
for (t in levels(as.factor(utility_df_treatment_aic_analysis$treatment_id))) {
  ...
  aic_table <- aic_table %>% 
    rbind.data.frame(data.frame(Treatment = treatment, 
                                Delta_AIC = delta_aic))
}
...
ggplot(data = aic_table, aes(Delta_AIC, reorder(Treatment,-Delta_AIC))) + 
