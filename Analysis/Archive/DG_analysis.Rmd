...
master_aic_analysis <- master %>% 
  filter(Game_type %in% c("Donation Game")) %>%
  subset.data.frame(subset = !(Standard_game %in% c("N")) & 
                      Choice_Method != "OnlyNorms" & 
                      StatusTreatment_Roma == "6-Complete" & 
                      Baseline %in% c(NA, 1)) %>% 
...
## MODEL BY STATA
```{r stata_output_model, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment=""}
getwd()
stata_output_model <- read_excel("stata output model DON.xlsx")

stata_output_model <- stata_output_model %>%
  mutate(weight_SE=(n_obs)^2) 

average_coeff_stata <- stata_output_model %>% summarise(across(s_delta:cr_sigma, ~weighted.mean(., w = n_obs, na.rm=T)))
average_SE_stata <- stata_output_model %>% summarise(across(s_delta_SE:cr_sigma_SE, ~weighted.mean(.^2, w = weight_SE, na.rm=T)))

stata_output_model <- stata_output_model %>%
   add_row(treatment_id="Average", average_coeff_stata , sqrt(average_SE_stata), n_obs=sum(stata_output_model$n_obs), weight_SE=sum(stata_output_model$weight_SE))
```
...
## social expectation model
plot_s_delta <- ggplot(stata_output_model, aes(x=s_delta,y=treatment_id)) + 
...
plot_s_gamma <- ggplot(stata_output_model, aes(x=s_gamma,y=treatment_id)) + 
...
# inequality aversion model
plot_cr_rho <- ggplot(stata_output_model, aes(x=cr_rho,y=treatment_id)) + 
...
plot_cr_sigma <- ggplot(stata_output_model, aes(x=cr_sigma,y=treatment_id)) + 
