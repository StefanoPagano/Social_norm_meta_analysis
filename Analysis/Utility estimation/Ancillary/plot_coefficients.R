...
plot_coeff_social_generator_uncertainty <- function(data){
  ...
  plot_gamma <-  ggplot(data, aes(x=basegammaNU,y=treatment_id)) + 
    geom_pointrange(aes(xmin=basegammaNU-1.96*se_basegammaNU,xmax=basegammaNU+1.96*se_basegammaNU), shape=20) +
    xlab(expression(gamma)) +
    theme_light() +
    theme(#axis.text.y=element_blank(),
          #axis.title.y=element_blank(),
  ...
  plot_eta <-  ggplot(data, aes(x=baseetaNU,y=treatment_id)) + 
    geom_pointrange(aes(xmin=baseetaNU-1.96*se_baseetaNU,xmax=baseetaNU+1.96*se_baseetaNU), shape=20) +
    xlab(expression(eta)) +
    theme_light() +
    theme(axis.text.y=element_blank(),
          axis.title.y=element_blank(),
  ...
  # model with interaction term
    ...
    geom_pointrange(aes(xmin=gammaNU-1.96*se_gammaNU,xmax=gammaNU+1.96*se_gammaNU), shape=20) +
    xlab(expression(gamma)) +
    theme_light() +
    theme(#axis.text.y=element_blank(),
          #axis.title.y=element_blank(),
  ...
  plot_eta_i <-  ggplot(data, aes(x=etaNU,y=treatment_id)) + 
    geom_pointrange(aes(xmin=etaNU-1.96*se_etaNU,xmax=etaNU+1.96*se_etaNU), shape=20) +
    xlab(expression(eta)) +
    theme_light() +
    theme(axis.text.y=element_blank(),
          axis.title.y=element_blank(),
  ...
  plot_interaction <-  ggplot(data, aes(x=sucaNU,y=treatment_id)) + 
    geom_pointrange(aes(xmin=sucaNU-1.96*se_sucaNU,xmax=sucaNU+1.96*se_sucaNU), shape=20) +
  ...
  plot_nu_i <- ggarrange(plot_gamma_i, plot_eta_i, plot_interaction, common.legend=T, legend="bottom", widths = c(8,6))
  
  return(list(plot_nu, plot_nu_i))
}

## Plot coefficients -----------
stata_output_model <- model_df_generator("Utility estimation/Output/Logs/MODEL_DG.csv", 0)
...
stata_output_model_u <- model_df_generator("Utility estimation/Output/Logs/uncertainty_MODEL_DG.csv", 1)
