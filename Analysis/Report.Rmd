...
comments on individual analysis
        - $\alpha < 0$
...
$$Strength = {NE_{Consistency}+NE_{Specificity^-}+NE_{Specificity^+}}$$

where:

**Specificity** indicates how appropriate is prevailing social norm relative to other possible action

* $NE_{Specificity^+} = \frac{{\sum_{i=1}^{N-1} max(KW)-KW_i}}{(N-1)[max(KW)-min(KW)]}$ $\forall KW_i > 0$

* $NE_{Specificity^-} = \frac{{\sum_{i=1}^{N-1} max(|KW|)-|KW|_i}}{(N-1)[max(|KW|)-min(|KW|)]}$ $\forall KW_i < 0$
...
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment=""}
cor.test(master$Norm_Strength_old, master$Delta, method= "spearman", exact = F)
...
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment=""}
summary(master$Sd_Avg_NE)
ggplot(data=master%>% subset.data.frame(Choice_Method!="OnlyNorms"), aes(x=Sd_Avg_NE)) +
geom_histogram(bins = 7, col=I("black")) + ggtitle("Histogram of Specificity (SD_Avg_NE") + xlab("Specificity") + facet_wrap(~Game_type)
```

```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment=""}
cor.test(master$Sd_Avg_NE, master$Delta, method= "spearman", exact = F)
ggplot(data=master%>% subset.data.frame(Choice_Method!="OnlyNorms"), aes(x=Sd_Avg_NE, y=Delta)) + geom_point() + geom_smooth(method = "lm") + stat_cor(method = "spearman") #geom_text(aes(label=PaperID))
...
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment=""}
cor.test(master$consistency_old, master$Delta, method= "spearman", exact = F)
