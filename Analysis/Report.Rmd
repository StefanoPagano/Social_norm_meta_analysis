...
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment=""}
i=i+1
Bas115 = individual_db_w %>% subset.data.frame(paper_id=="2020Bas115" & Game_type=="DG") %>% mutate(payoff=endowment-scenarios)
clogit.bas <- clogit(A~ payoff + KW_Normative + strata(subject_id), data = Bas115)
summary(clogit.bas)
dataframe_betas[i,] <- c("2020Bas115",clogit.bas$coefficients["payoff"],clogit.bas$coefficients["KW_Normative"],sqrt(clogit.bas$var[1,1]),sqrt(clogit.bas$var[2,2]))
```
...
summary(clogit.kim)
dataframe_betas[i,] <- c("2016Kim003",clogit.kim$coefficients["payoff"],clogit.kim$coefficients["KW_Normative"],sqrt(clogit.kim$var[1,1]),sqrt(clogit.kim$var[2,2]))
```
...
#mlogit.kim <- mlogit(A~payoff+KW_Normative|0, data = Kim003_m)
#summary(mlogit.kim)#$CoefTable[1,2] ### SUCA 2beta/alpha monetary trade off from -1 to 1 
clogit.kim_ug <- clogit(A~payoff+KW_Normative +strata(subject_id), data = Kim003)
summary(clogit.kim)
dataframe_betas[i,] <- c("2016Kim003",clogit.kim$coefficients["payoff"],clogit.kim$coefficients["KW_Normative"],sqrt(clogit.kim$var[1,1]),sqrt(clogit.kim$var[2,2]))
```
...
#summary(mlogit.her)
summary(clogit.her)
dataframe_betas[i,] <- c("2018Her061",clogit.her$coefficients["payoff"],clogit.her$coefficients["KW_Normative"],sqrt(clogit.her$var[1,1]),sqrt(clogit.her$var[2,2]))
...
#summary(mlogit.cha)
dataframe_betas[i,] <- c("2019Cha026",clogit.cha$coefficients["payoff"],clogit.cha$coefficients["KW_Normative"],sqrt(clogit.cha$var[1,1]),sqrt(clogit.cha$var[2,2]))
```

#### All papers results
```{r echo=F, message=FALSE, warning=FALSE}
opts <- options(knitr.kable.NA = "")
kbl(dataframe_betas %>% mutate_at(.vars = vars(alpha:var_beta), as.numeric) %>% mutate(MRS = 2*as.numeric(beta)/as.numeric(alpha)), digits = 2, booktabs = T, align = "lccccc", centering = F, caption = "Estimated Parameters") %>%
  kable_styling(position = "left", full_width = F) %>%
  column_spec(1, bold = T) 
```
...
<center>
$U_{ij} = \alpha \pi_j + \beta S_{ij} + \eta SD(S_{ij})_i$ 
</center>

while for 2., we use $S_j=\Sigma S_{ij}/N$ from the external sample with which social norms were elicited. Hence the model becomes:

<center>
$U_{ij} = \alpha \pi_j + \beta S_{j} + \eta SD_j$ 
</center>
...
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment=""}
i=1
SDi= Bas115 %>% group_by(subject_id) %>% summarise(sdi=sd(KW_Normative))
Bas115_plus <- Bas115 %>% merge.data.frame(SDi)
summary(Bas115_plus$sdi)
Bas115_plus_m <- dfidx(Bas115_plus %>% mutate(scenarios=as.factor(scenarios)),
            idx = c("subject_id","scenarios"), 
            idpkg = "mlogit")

clogit.bas_plus <- clogit(A~ payoff + KW_Normative+ KW_Normative:sdi + KW_Normative:age + KW_Normative:gender + strata(subject_id), data = Bas115_plus)
summary(clogit.bas_plus)
