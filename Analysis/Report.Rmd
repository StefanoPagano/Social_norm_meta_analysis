...
* $NE_{Specificity} = \frac{\sum_{i=1}^N max(KW_i)-KW_i}{N-1}$ ??????
...
ggplot(data=master%>% subset.data.frame(Choice_Method!="OnlyNorms"), aes(x=Norm_Strength, y=Delta, color = Game_type)) + geom_point() + geom_text(aes(label=PaperID)) # geom_smooth(method = "lm") + stat_cor(method = "spearman") 
