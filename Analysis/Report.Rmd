...
# read data 
master <- read.csv("File_DB/Output/Treatment.csv") %>% mutate(Macro_game_type = ifelse(Game_type %in% c("DG","Donation Game"), "DG and Donation Game", Game_type)) %>% 
  subset.data.frame(subset = PaperID !="2017Del037")
...
master = master %>% mutate(Norm_Strength = 1/(sqrt(Var_NE)+Sd_Avg_NE),
                           Delta_new = (Avg_NE-Avg_coop))
...
### Delta and Strength Comparison between DG and UG
We test the hypothesis whether delta in DG and Donation Game is lower then UG. In this case, the p-value of Mann-Whitney-Wilcoxon Test is 0.0242, therefore we can reject the null hypothesis (at .05 significance level).
Furthermore, we test the hypothesis whether Norm Strength in DG and Donation Game is lower then UG. 
...
hp4 <- master %>% 
  ...
  summarise(Coop = mean(Avg_coop, na.rm = T), 
            Sd_Coop = mean(sqrt(Var_coop), na.rm = T), 
            Avg_Norm = mean(Avg_NE, na.rm = T), 
            Delta = mean(Delta_new, na.rm = T), 
            Strength = mean(Norm_Strength, na.rm = T),
            n = n()) %>% arrange(Coop)

master_test <- master %>% subset.data.frame(select = c(Game_type, Delta_new, Norm_Strength, Macro_game_type), subset = Game_type %in% c("DG", "UG"))

wilc_delta = wilcox.test(Delta_new ~ Macro_game_type, data=master_test)
wilc_strength = wilcox.test(Norm_Strength ~ Macro_game_type, data = master_test)

hp4plus <- hp4 %>% add_row(Macro_game_type = "Difference (Wilcoxon p-value)", Delta = wilc_delta$p.value, Strength = wilc_strength$p.value)

opts <- options(knitr.kable.NA = "")
kbl(hp4plus, digits = 4, booktabs = T, align = "lccccc", centering = F) %>%
