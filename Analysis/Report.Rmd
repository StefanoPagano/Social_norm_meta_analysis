...
master = master %>% mutate(consistency=sqrt(Var_NE)/max_sigma,
                           Norm_Strength = consistency + specificity,
...
ggplot(data=master%>% subset.data.frame(Choice_Method!="OnlyNorms"), aes(x=specificity)) +
geom_histogram(bins = 7, col=I("black")) + ggtitle("Histogram of Specificity") + xlab("Specificity") + facet_wrap(~Game_type)
```
...
master_test <- master %>% subset.data.frame(select = c(Game_type, Avg_coop, Avg_NE, Delta, Norm_Strength, Macro_game_type, specificity, consistency), subset = Game_type %in% c("DG", "UG"))
...
hp4_SP_CS <- master %>% 
  ...
  group_by(Game_type) %>% 
  summarise(Specificity = mean(specificity, na.rm = T), 
            Consistency = mean(consistency, na.rm = T)) %>% 
  arrange(Game_type)

F_test_spec <- wilcox.test(master_test$specificity ~ master_test$Game_type)
