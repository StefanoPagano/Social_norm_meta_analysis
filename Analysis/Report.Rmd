...
master = master %>% mutate(consistency=sqrt(Var_NE)/max_sigma,
                           Norm_Strength = consistency + specificity_plus + specificity_min,
...
* **Cooperation** ($Coop_{avg}$) in DG is lower then UG.
...
* **Norm Strength** in DG is lower then UG.
...
```{r echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, comment=""}
i=1
SDi= Bas115_dg %>% group_by(scenarios) %>% summarise(sdi=sd(KW_Normative))
...
model <- clogit(A~payoff+KW_Normative*sdj + strata(subject_id), data = Kim003_dg)
summary(model)
...
```{r}
individual_db_b %>% merge.data.frame(master%>% rename(TreatmentCode=treatment_id))
```
