...
master <- read.csv("File_DB/Output/Treatment.csv") %>% 
  ...
  subset.data.frame(subset = Standard_game != "N")

# se togliamo tutti i giochi non standard abbiamo problemi con l'ipotesi HP3... per i dg la relazione non è più significativa...
...
$$Strength = {1 \over NE_{Consistency}+NE_{Specificity}}$$

where:

* $NE_{Specificity}$ is the standard deviation of Krupka-Weber appropriateness score (for each treatment accounts for the mean of subjects' scores)

* $NE_{Consistency}$ accounts for the variability among appropriateness scores of individuals with respect to the norm in the game (e.g., if the norm in the game is 5, we compute the standard deviation among the appropriateness scores
...
master_test <- master %>% subset.data.frame(select = c(Game_type, Avg_coop, Avg_NE, Delta, Norm_Strength, Macro_game_type), subset = Game_type %in% c("DG", "UG"))
...
kbl(hp4plus_mean, digits = 4, booktabs = T, align = "lccccc", centering = F, caption = "Average and Minn-Whitney-Wilcoxon Test") %>%
...
hp4_SP_CS <- master %>% 
  subset.data.frame(subset = Macro_game_type %in% c("DG and Donation Game", "UG")) %>%
  group_by(Macro_game_type) %>% 
  summarise(Specificity = mean(Sd_Avg_NE, na.rm = T), 
            Consistency = mean(sqrt(Var_NE), na.rm = T)) %>% 
  arrange(Macro_game_type)

opts <- options(knitr.kable.NA = "")
kbl(hp4_SP_CS, digits = 4, booktabs = T, align = "lccccc", centering = F, caption = "Consistency e Specificity") %>%
  kable_styling(position = "left", full_width = F) %>%
  column_spec(1, bold = T)
```
