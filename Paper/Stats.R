social_norms <- read_excel("Social Norms meta.xlsx", sheet = "ALL")
...
n_treat_ok <- nrow(distinct(social_norms %>% filter(Game_type %in% c("DG", "Donation Game", "ToG") & StatusTreatment_Roma != "1-Not eligible treatment") %>% select(treatment_id)))
...
target <- social_norms %>% filter(Game_type %in% c("DG", "Donation Game", "ToG") & StatusTreatment_Roma != "1-Not eligible treatment")
