...
tgs=read.csv("/TG_Data_bysubj.txt", sep="\t")
...
dg_dta_coop <- dg %>% subset.data.frame(select = coldg, subset = role == 1) %>% mutate(endowment = 16, cooperation = sent/endowment) %>% 
  summarise(mean_cooperation = mean(cooperation))

dg_dta_var <- dg %>% subset.data.frame(select = coldg, subset = role == 1) %>% mutate(endowment = 16, cooperation = sent/endowment) %>% 
  summarise(mean_cooperation = var(cooperation))
...
dg_norms <- norms1[, c(1, 3, 4, 11:19)] %>% summarise(sum(norms1[,11]))
label_col = paste("answer.",1,".", sep = "")
label_count = 1
